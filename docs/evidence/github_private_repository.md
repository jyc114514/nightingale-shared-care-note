# Private GitHub publication evidence - 2026-08-27

- Repository: https://github.com/jyc114514/nightingale-shared-care-note
- Owner/name: `jyc114514/nightingale-shared-care-note`
- Visibility: GitHub repository page displayed `Private` after creation and after push.
- Default branch: `main`.
- Push method: existing Git Credential Manager authentication; no password-file or token was
  read, entered, printed, or stored by the project.
- Initial pushed checkpoint: `9a4ffa4`.
- Local/remote verification at push: `origin/main` matched local `9a4ffa4`.
- Repository creation: one empty private repository; no README, license, or GitHub-generated
  ignore file was added because the local repository already contained them.
- No collaborators, Pages site, Release, or visibility-changing Action was created.

The repository must remain private until 2026-08-28 18:00 Asia/Shanghai. Future pushes must repeat
the final secret scan and verify that no deployment secrets, model cache, database, runtime log,
or raw uploaded audio is included.

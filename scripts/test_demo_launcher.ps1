...
$relativeStartScript = "scripts\start_demo.ps1"
$relativeStopScript = "scripts\stop_demo.ps1"
$relativeRuntimeDirectory = "artifacts\local-runtime\smoke"
...
try {
  ...
  $launcherProcess = Start-Process -FilePath "powershell.exe" -ArgumentList @(
    ...
    "-NoExit",
    "-File", $relativeStartScript,
    "-DemoPassword", $password,
    "-NoBrowser",
    "-Setup",
    "-ProviderOverride", "fixture",
    "-TimeoutSeconds", "45",
    "-RuntimeDirectory", $relativeRuntimeDirectory
  ) -WorkingDirectory $scriptRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput $launcherOutput -RedirectStandardError $launcherError
  ...
  $secondOutput = powershell.exe -NoProfile -ExecutionPolicy Bypass -File $relativeStartScript -NoBrowser -ProviderOverride fixture -TimeoutSeconds 10 -RuntimeDirectory $relativeRuntimeDirectory 2>&1 | Out-String
  ...
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $relativeStopScript -RuntimeDirectory $relativeRuntimeDirectory
  ...
  try {
    if (Test-Path -LiteralPath $runtimePath) {
      & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $relativeStopScript -RuntimeDirectory $relativeRuntimeDirectory *> $null
    }
  }
  finally {
    Stop-LauncherHost
